...
#############################################################################
registry:                                                  #认证和检索配置
  maxenrollments: -1                                       #用于登记的密码/口令最大可重复使用的次数(即一个用户可以被登记多少次), -1为无限制, 0为不允许任何用户登记
...
id:                                                          #向ca-server注册身份的配置
  ...
  type:                                                      #身份的类型
  affiliation:                                               #身份所属的组织
  maxenrollments: 0                                          #身份的密码/口令最多的重用次数(即一个用户可以被登记多少次), 0表示使用ca-server的设置, 这个值必须小于等于ca-server的配置值
  attributes:                                                #身份的一系列属性
   # - name:
   #   value:
enrollment:                                                  #登记的身份
...
>   - id.secret: 手动指定新注册身份的密码, 此密码必须满足该身份的maxenrollments和ca-server的maxenrollments属性的次数要求
